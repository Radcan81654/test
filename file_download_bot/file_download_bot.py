...
async def upload_progress(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if upload_info:
        ...
        await update.message.reply_text(f"Uploading {upload_info['file_name']}: 0 % completed.")
    ...
    try:
        if download.is_metadata:
            ...
            if download.followed_by_ids:
                ...
                await update.message.reply_text(f"Metadata downloaded.")
                #################################################################################################
                ...
                if download_status == 'error':
                    ...
                    for dl in active_downloads:
                        if dl.status=='complete':
                            if dl.gid==download.gid:
                                continue
                            if download.info_hash==dl.info_hash:##只能处理磁力链接的重复下载，解决不了种子文件提供的重复下载
                    ...
                    if completed_download:
                        ...
                        if completed_download.is_metadata:
                            ...
                            print( f"Found an existing active download for {real_complete_download}. " f"Binding current request to this download.")
                            ...
                            print( f"Found an existing active download for {completed_download}. " f"Binding current request to this download.")
